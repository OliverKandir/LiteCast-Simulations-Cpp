...
const int SLOT_FRAME = 15;
const int SUB_SLOTS = 47;
...
vector<int> getWinnerNode(vector<int> contenders, map<int, vector<Data*>> &DataTransmitted, int receiver, int subslot)
{
    ...
    for (int contender : contenders){
        ...
        if (Data->data != firstData->data or Data->TTL != firstData->TTL){
            sameData = false;
            break;
        }
    }
...
void updateReceivers(int receiver, Data* receivedData, int subslot, int timeslot){
    ...
    if(receivedData->TTL-1 < 0) return;
    ...
    if(!acquired_for_the_subslot){ //not acknowleged for the subslot
        Nodes[receiver]->PacketsFor[subslot].push(new Data(receivedData->data, ntxForwarding, receivedData->TTL-1));
    ...
    for (auto id: IDs){
        ...
        for(int i = 0; i < SUB_SLOTS; i++){
            ...
            if(!q.empty()){
                ...
                queue<Data*> temp = q;
                while(!temp.empty()){
                    cout << "(" << temp.front()->data << "," << temp.front()->ntx << "," << temp.front()->TTL << ") ";
                    temp.pop();
                }
                cout << "\n";
            }
...
void display_NodeLayer_and_HopCount(){    
    for(auto [id, node]: Nodes){
        cout << "Node " << id << " has parity: " << node->layer << endl;
    }
}
...
Nodes[47] = new Node(47, NodePositions[46].first, NodePositions[46].second, {41, 46});
    ...
    for (auto& [id, node] : Nodes)
    {
        ...
        node->PacketsFor[node->ID%SUB_SLOTS].push(new Data(node->ID, ntxOriginator, TTL));
    ...
    for (auto &[id, node] : Nodes){
       K_HOP_Neighbors[id] = Get_K_HOP_Neighbors(id, TTL);
    }
    ...
    for(int timeslot = 1; timeslot <= SLOT_FRAME; timeslot++ ){
