...
bool hasAcquiredAllKHopNeighbors(int nodeID, map<int, set<int>> K_HOP_Neighbors)
{
    Node* node = Nodes[nodeID];

    for (int neighborID : K_HOP_Neighbors[nodeID])
    {
        if (!node->DataAcquired.count(neighborID))
            return false;
    }
...
//========================================================================================================
SimulationResult runSimulation(int SUB_SLOTS){
    ...
    for(int timeslot = 1; timeslot <= TIME_SLOT; timeslot++ ){
        ...
        for(int subslot = 0; subslot < SUB_SLOTS;  subslot++){
            ...
            for(int receiver : nodeids){
                ...
                if (!Nodes[receiver]->Time_Taken and hasAcquiredAllKHopNeighbors(receiver, K_HOP_Neighbors)){
                    Nodes[receiver]->Time_Taken = Time_Elapsed * SUBSLOT_TIME_MS;
                }      
            }
        }
    ...
    for (auto &[id, node] : Nodes)
    {
        ...
        {
            node->Time_Taken = TIME_SLOT * SUB_SLOTS * SUBSLOT_TIME_MS;
        }
    }

    //------------------------Calculating Reliability Accross all nodes--------------------------------    
    ...
    averageCumulative *= 100;

    //------------------------Calculating Latency Accross all nodes--------------------------------
    double averageLatency = 0.0;

    for (auto &[id, node] : Nodes)
    {
        averageLatency += node->Time_Taken;
    }

    averageLatency /= Nodes.size();

    //returning results

    SimulationResult result;

    result.subslots = SUB_SLOTS;
    result.averageCumulative = averageCumulative;
...
int main()
{
    ...
    cout << "Ran Simulation for " << TIME_SLOT << " Time Slots" << endl;
    cout << endl<< "Displaying LC-" << LC << " Results" << endl;
    cout << endl << left
         << setw(10) << "Subslots"
         << setw(18) << "Reliability(%)"
         << setw(15) << "Latency(ms)"
    ...
    for (int subslots = 1; subslots <= 50; ++subslots)
    {
        SimulationResult result = runSimulation(subslots);

       cout << left
            << setw(10) << subslots
            << setw(18) << fixed << setprecision(2)
            << result.averageCumulative
