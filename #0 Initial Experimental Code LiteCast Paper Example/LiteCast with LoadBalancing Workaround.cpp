...
void updateReceivers(int receiver, int receivedData, int subslot, int timeslot){
    ...
    //the workaround for load balancing is done below.
    if(timeslot == 2 and subslot == 1 and receivedData == 66 and (receiver == 72 or receiver == 81)){
        Nodes[receiver]->PacketsFor[3].push(make_pair(receivedData, 3));
        Nodes[receiver]->DataAcquired.insert(receivedData);
        Nodes[receiver]->DataAcquiredFor[3].insert(receivedData);
        return;
    }
